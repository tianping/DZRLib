...
#' }
seurat_barplot_metadata <- function(seurat_obj, 
                                    x_var, 
                                    fill_var = NULL, 
                                    save_path = "cell_counts_barplot.png",
                                    width = 12,
                                    height = 8,
                                    dpi = 300,
                                    angle = 45,
  ...
  if (!x_var %in% colnames(seurat_obj[[]])) {
    stop(paste("x_var '", x_var, "' not found in metadata columns. Available columns: ", 
  ...
  if (is.null(fill_var)) {
    ...
    if (!fill_var %in% colnames(metadata)) {
      ...
      stop(paste("fill_var '", fill_var, "' not found in metadata columns. Available columns: ", 
